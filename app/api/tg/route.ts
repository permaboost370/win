...
const PROMPT =
  "Modify only the hair and eyes of the character in the first image. Replace the existing hair with the long swept-back bright blonde hair shown in the second reference image — same blonde color, same swept-back volumized styling, same cartoon shape. Place the black wayfarer-style sunglasses from the second reference image over the character's eyes. Do not redraw, modify, remove, replace, or reinterpret any other part of the first image — face shape, skin tone, expression, mouth, ears, neck, body, clothing, suit, tie, accessories, props, and background must remain identical to the original. The only changes are: blonde hair on top and black wayfarer sunglasses over the eyes. Match the exact art style of the first image — if it's a photograph render the hair and sunglasses photorealistically; if it's illustration, painting, anime, comic, or pixel art, match its linework, palette, and shading. Take only the hair color and shape and the sunglasses from the second reference image; do not copy the face, skin, body, suit, tie, or background from it. Do not output Donald Trump.";
