...
const PROMPT =
  "Make only two changes to the character in the first image: (1) replace the existing hair with hair that copies the EXACT same silhouette, swept-back volumized shape, partline, length, and bright blonde color as the hair in the second reference image; (2) place sunglasses over the eyes that copy the EXACT same black wayfarer frame shape, lens shape, frame thickness, and proportions as the sunglasses in the second reference image. The new hair and sunglasses must visually match the reference image's hair and sunglasses — not generic blonde hair or generic sunglasses, but the specific ones shown in reference. Do not redraw, modify, remove, replace, or reinterpret any other part of the first image — face shape, skin tone, expression, mouth, ears, neck, body, clothing, suit, tie, accessories, props, and background must remain identical to the original. Match the rendering style of the first image: if it is a photograph, render the new hair and sunglasses photorealistically; if it is illustration, painting, anime, comic, pixel art, or 3D, match its linework, palette, shading, and brush style. Take only the hair shape/color and the sunglasses shape from the second reference image — do NOT copy the face, skin, body, suit, tie, or background from it, and do NOT output Donald Trump.";
